...
def run_episode(envs, client, num_envs=4):
    ...
    while done or tot_rew[0]==30:
        print('Start')
        start = time.time()
        actions = [action(ep, obs) for ep, obs in zip(eps, obsvs)]
        print('Action', time.time() - start)
        start = time.time()
        transitions = [step(env, action) for env, action in zip(envs, actions)]
        ...
        start = time.time()
        [log_return_and_end(ep , rew, obs, d) for ep, rew, obs, d in zip(eps, rews, obsvs, done)]
        print('LogEnd', time.time() - start)
        eps = [ep for ep, d in zip(eps, done) if not d]
        obsvs = [tr[0] for tr, d in zip(transitions, done) if not d]
